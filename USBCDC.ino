 ...
 * @li Connect the C5535/15/17 EVM with a PC via USB cable
 ...
 * 05-Mar-2013 Modified for USB Chapter 9 compliance tests
 * 15-Mar-2013 Fixed issue with receive buffer allocation  
 * ============================================================================
 */
...
#define CSL_USB_DEBUG_PRINT Serial.println
#else
...
Uint16	usbDataBufferRxWork[CSL_USB_CDC_DATA_BUF_SIZE/2];
Uint16	bytesRem;
Uint16	devAddr;
Uint16	saveIndex;
Uint16	endpt;
...
Uint16    cfgDesc[40] =	{
				...
				0xC003, // Attributes (Bus-Powered and Self-Powered) | Configuration string index
				0x0932,	// Size of interface 0 | max power consumption (2X mA)
							...
							0x0507, // USB endpoint descriptor type | Endpoint descsriptor size
							0x0300|0x80|CSL_CDC_INTR_IN_EP, // Attributes (3: Interrupt) | INTR EP IN
							0x0040, // Maximum packet size 512
...
Uint16    cfgDescFS[40] =	{
							...
							0x0034, // Total length of data for this configuration
							0x0102, // Index value of this configuration | number of interfaces in this configuration
							0xC003, // Attributes | Configuration string index
							0x0932,	// Size of interface 0 | max power consumption (2X mA)
                         	// Interface 0 descriptor
                         	0x0004, // Interface Number | Interface descriptor type
                         	0x0100, // Number of endpoints in this interface | Alternate setting number
...
CSL_Status CSL_usbCdcTest(void)
{
	...
	*(ioport volatile int *)0x1c03 = 0x0;
	...
	memset((void*)usbDataBufferRxWork, 0x00, sizeof(usbDataBufferRxWork));
	...
	CDC_AppHandle.rxWorkBufPtr   = usbDataBufferRxWork;
	CDC_AppHandle.rxWorkBufIdx = 0;	
	CDC_AppHandle.pId    = pId;
	CDC_AppHandle.vId    = vId;

	/* All Function Handlers in the CDC module handle need to be Initialized */
	CDC_AppHandle.bulkInCallback          = (void (*)(...))appBulkInCallback;
	CDC_AppHandle.bulkOutCallback         = (void (*)(...))appBulkOutCallback;
	CDC_AppHandle.intrInCallback          = (void (*)(...))appIntrInCallback;
	...
	gUsbDevHandle = pHandle->usbDevHandle;

	// Pass the endpoint object pointers from CDC handle to USB context
	pContext->hEpHandleArray[0] = cdcClassStruct.ctrlHandle.ctrlOutEpHandle;
	pContext->hEpHandleArray[1] = cdcClassStruct.ctrlHandle.ctrlInEpHandle;
	pContext->hEpHandleArray[2] = cdcClassStruct.cdcHandle.bulkOutEpHandle;
	pContext->hEpHandleArray[3] = cdcClassStruct.cdcHandle.bulkInEpHandle;
	pContext->hEpHandleArray[4] = cdcClassStruct.cdcHandle.intrOutEpHandle;
	pContext->hEpHandleArray[5] = cdcClassStruct.cdcHandle.intrInEpHandle;

	/* Set the parameters */
	status = USB_setParams(gUsbDevHandle, pContext->hEpHandleArray, FALSE);
	if(status != CSL_SOK)
	{
		Serial.print("USB Set params failed\n");
		return(result);
	}
	...
	if(status != CSL_SOK)
	{
		Serial.print("USB Connect failed\n");
		return(result);
	}
	...
	// The echo loop
        Serial.print("Remove USB CDC Device Cable\n");
	while(stopRunning != TRUE)
	{
		            ...
		            CSL_USB_DEVCTL_VBUS_ABOVEVBUSVALID))
		{
			Serial.print("\nUSB Cable Disconnected!!\n");
                        Serial.print("Reconnect USB CDC Device Cable\n");
			while(CSL_FEXT(usbRegisters->DEVCTL, USB_DEVCTL_VBUS) !=
		                   CSL_USB_DEVCTL_VBUS_ABOVEVBUSVALID);

			Serial.print("USB Cable Connection Detected!!\n");
		...
		if (uartDataSize>0)
		{
			do
			{
				...
				retNum = CDC_putData(pContext, &CDC_AppHandle, uartData, uartDataSize);
                                ...
                                disp.print("Received");
                                disp.setline(1);
                                Serial.println(uartDataSize);
                                for(int i = 0; i < uartDataSize; i++)
                                {
                                  Serial.print((char)uartData[i]);
                                  disp.print((char)uartData[i]);
                                }
			} while (retNum==0);
		}
	}
...
interrupt void usb_isr(void)
{
	...
	if((pContext->dwIntSourceL != FALSE) || (pContext->dwIntSourceH != FALSE))
	{
		...
		if(pContext->dwIntSourceH & CSL_USB_GBL_INT_RESET)
		{
			...
			if(pContext->hEpHandleArray[2]->epNum == CSL_USB_IN_EP1)
			{
		...
		{
			usb_int_resume++;
			USB_setRemoteWakeup(gUsbDevHandle, CSL_USB_TRUE);
			status = USB_issueRemoteWakeup(gUsbDevHandle, TRUE);
			/* Give 10 msecs delay before resetting resume bit */
			USB_delay(CSL_USB_WAKEUP_DELAY);
			status = USB_issueRemoteWakeup(gUsbDevHandle, FALSE);
			if(status != CSL_SOK)
			{
				Serial.print("USB Resume failed\n");
			}
		}
		...
		if(pContext->dwIntSourceH & CSL_USB_GBL_INT_DEVDISCONN)
		{
			...
			if(status != CSL_SOK)
			{
				Serial.print("USB Disconnect failed\n");
			}
		}
		...
		{
			usb_int_suspend++;
			status = CSL_SOK;//USB_suspendDevice(gUsbDevHandle);
			if(status != CSL_SOK)
			{
				Serial.print("USB Suspend failed\n");
			}
		}
	}
...
static void USB_delay(Uint32    mSecs)
{
	...
	sysClk = 200000;  /* It is assumed that system is running at 100MHz */
...
void setup()
{
    ...
    Serial.begin(9600);
    Serial.print("USB Test\n");  
      disp.oledInit();
    disp.clear();
    disp.flip();
    disp.setline(1);

    disp.print("CDC Begin");    
	CSL_Status    result;

	Serial.print("USB Interrupt Test!!\n");

	result = CSL_usbCdcTest();

	if(result == CSL_USB_TEST_PASSED)
	{
		Serial.print("USB Test Passed!!\n");
	}
	else
	{
		Serial.print("USB Test Failed!!\n");
...
//    disp.print("Received");
//    disp.setline(1);
//    disp.print((char*)usbDataBufferRx);
//    flag = 0;
//  }
}
